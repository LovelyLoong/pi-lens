...
- **Mixed-validity multi-edits now use host-backed adaptive safe partial by default** — preflight classifies exact, unique candidates and original-indexed failures, rejects incomplete/overlapping plans, asks the read guard to authorize every candidate range, and narrows only the in-memory `tool_call` input. The native host edit tool then revalidates, serializes same-file writes, and produces the real diff/patch; pi-lens never writes from the guard hook. An early, minimal `tool_result` handler correlates by stable `toolCallId`, preserves native result fields, appends structured applied/failed indexes plus the preflight snapshot hash, emits a TUI/RPC warning, and clears state before the heavier diagnostics pipeline. Host rejection reports `partial_rejected` without claiming a commit; abort, turn-end, and session-shutdown paths clear pending plans. Any unprovable case falls back atomically. `--lens-atomic-multi-edit` / `edit.mixedValidityMode: "atomic"` is the kill switch; `--no-read-guard` bypasses adaptive classification and leaves the original batch to the host's atomic preflight. The old direct-writer module and `edit.partialApply` opt-in were removed. Contract, coordinator, config, read-guard, host-queue, race, cleanup, and integration coverage pin the behavior.
